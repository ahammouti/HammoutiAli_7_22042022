...
this.filterRecipes = [];
let searchedString = "banane";

this.filterRecipes = recipes.filter(recipe => {
    if (recipe.name.toLowerCase().includes(searchedString) || recipe.description.toLowerCase().includes(searchedString)) {
        if (!this.filterRecipes.includes(recipe)) {
            this.filterRecipes.push(recipe);
        }

        recipe.ingredients.forEach(ingr => {
            if (ingr.ingredient.includes(searchedString)) {
                if (!this.filterRecipes.includes(recipe)) {
                    this.filterRecipes.push(recipe);
                }
            }
